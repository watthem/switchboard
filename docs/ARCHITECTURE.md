...
Today's answers are "everything," "we don't know," and "nobody." Herald fixes this without requiring teams to rewrite their agents or adopt a new runtime.
...
Herald defines a minimal contract between any agent and the Herald control plane. The contract has three parts:

### 1. Policy In (Herald → Agent)

Herald writes a policy file that the agent's runtime reads. The format adapts to the runtime — JSON, YAML, TOML, environment variables. The content is the same:

```yaml
# Example: policy.yaml (written by Herald sidecar)
...
The agent reads this however its runtime prefers. A Python agent reads YAML. A Ruby agent reads JSON. A Go binary reads env vars. Herald doesn't care.

### 2. Events Out (Agent → Herald)
...
- **Network policies** — An L1 agent can reach Herald and nothing else.
- **Credential injection** — Secrets live in Herald, not in agent containers.
...
Each agent runs with a **sidecar** — a thin process that handles the Herald protocol on the agent's behalf. The sidecar:
- Pulls policy from Herald and writes it in the format the agent expects
- Forwards the agent's event output to Herald
...
1. **Deploy with sidecar** — Run your agent container alongside the Herald sidecar. The sidecar connects to Herald automatically.
...
| **Credential exfiltration** | Secrets live in Herald, not in agent containers. Agents receive processed data only. No agent can read another agent's environment. |
| **Lateral movement** | No direct agent-to-agent communication. All messages route through Herald. A compromised agent cannot discover or reach other agents. |
...
The `herald/` directory contains the first Herald implementation (FastAPI/Python). The `agents/` directory contains working agent configurations using OpenClaw as the runtime. This is the prototype that proves the architecture — not the only way to deploy.
